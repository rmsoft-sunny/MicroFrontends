...
import { SearchProps } from "../components/work-support-container";
import { useAxios } from "@/hooks/useAxios";
import { keepPreviousData, useInfiniteQuery } from "@tanstack/react-query";
import { toast } from "sonner";

export const useGetWorkSupportList = (search: SearchProps) => {
  const router = useRouter();
  const axios = useAxios();
  const queryFunction = async (pageParam: number) => {
    const response = await axios.get("/api/client/work/request/list", {
      params: {
        pageNum: pageParam + 1,
        name: search.workName,
        startDate: search.dateRange.from,
        endDate: search.dateRange.to,
        requestStatusTypeList: search.requestStatusTypeList,
      },
      paramsSerializer: {
        indexes: null,
      },
  ...
  const query = useInfiniteQuery({
    queryKey: ["GET", "work", "request", "list", search],
