import { InspectionStatusTypeEnum } from "@/enums/enum-list";
...
export const columns: ColumnDef<Type, unknown>[] = [
  {
    ...
    cell: ({ row }) => {
      const getStatus = InspectionStatusTypeEnum.get(row.original?.taskStatus);
      ...
      return (
        <div className="w-36 overflow-hidden truncate text-center">
          {row.original?.taskStatus ? (
            ...
            >
              {InspectionStatusTypeEnum.get(row.original.taskStatus)}
            </p>
          ) : (
            "-"
          )}
